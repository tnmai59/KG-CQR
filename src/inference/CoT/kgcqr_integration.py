...
def add_triplet_context_to_question(question):
	global KG
	global embeddings
	global summary_embeddings
	
	topk_indices_sorted = query_triplet_topk(question)
	relations = [dct_mapping_triplet[x] for x in topk_indices_sorted]
	T = format_T(relations)
	try:
		...
		if T == []:
			...
			H = relevance_guided_path_addition_beam(KG, T, embeddings, summary_embeddings, 20, 20, 2)
...
if __name__ == "__main__":
	...
	summary_embeddings = dict(
			zip(
				list(all_summaries),
				embeddings.embed_documents(list(all_summaries))
			)
		)
